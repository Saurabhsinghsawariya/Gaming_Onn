...
var FPS = 60; // Increased for smoother animation
var jump_amount = -6; // Reduced from -10 for smaller jumps
var max_fall_speed = +8; // Reduced from +10 for more control
var acceleration = 0.6; // Reduced from 1 for more floating feel
var pipe_speed = -2;
...
let lastTapTime = 0;
const MIN_TAP_INTERVAL = 100; // Minimum time between taps in milliseconds
function Got_Player_Input(MyEvent) {
    ...
    switch (game_mode) {
        ...
        case "over": {
            if (currentTime - time_game_last_running > 1000) {
                reset_game();
                game_mode = "running";
            }
            break;
...
function make_bird_slow_and_fall() {
    // Add air resistance for smoother falling
    if (bird.velocity_y < max_fall_speed) {
        bird.velocity_y = bird.velocity_y + (acceleration * (bird.velocity_y > 0 ? 0.8 : 1));
    }
    
    // Add slight hover effect
    if (Math.abs(bird.velocity_y) < 2) {
        bird.velocity_y += Math.sin(Date.now() / 200) * 0.1;
    }
...
function make_bird_tilt_appropriately() {
    // Smoother tilting animation
    if (bird.velocity_y < 0) {
        // Tilting up when jumping
        bird.angle = Math.max(-20, bird.velocity_y * 2);
    } else if (bird.angle < 70) {
        // Gradual tilting down when falling
        bird.angle = Math.min(70, bird.angle + (bird.velocity_y * 0.5));
    }
}
